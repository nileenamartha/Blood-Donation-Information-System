	  ...
	  if(mysqli_num_rows($result))
		{
			while($data=mysqli_fetch_array($result))
			{
				...
				$sql1="insert into notification_donor_info values(null,'$lid','$nid','$name','$bloodgroup','$phone_no','$email','$gender','$state','$district','$location',curdate(),'accepted')";
		  ...
		  if($result1)
			{
				...
				if($result2)
				{
				$sql3="update notification_donor_info set status='accepted' where id='$nid' and l_id='$lid'";
				$result3=mysqli_query($conn,$sql3);
				}
				?>
				<script>
				alert("Request successfully accepted");
				window.location="example.php?na=<?php echo $nid?>"
				</script>
				<?php
			}
		}
